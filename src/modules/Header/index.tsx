...
const Header = ({ pageCategory }:LayoutPropsType) => {
  const isShow = pageSet[pageCategory] ? 'flex' : 'hidden'

  return (
    <header className={`${isShow} bg-white absolute left-1/2 top-60 z-20 max-w-full -translate-x-1/2 transform items-center rounded-25 gap-60 px-32 py-16`}>
      <NavBar />
      <div className="relative flex items-center gap-38">
        ...
        <Link href="/login" className="shadow-headerIcon hover:shadow-none transform transition-shadow duration-300 ease-in-out hover:transform hover:translate-x-3 hover:translate-y-3 rounded-full">
