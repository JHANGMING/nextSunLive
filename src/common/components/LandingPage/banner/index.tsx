    ...
    <section className="relative mb-120">
      ...
      >
        {bannerImgData.map((data) => (
          <SwiperSlide key={data.alt}>{({ isVisible }) => <Image src={data.src} alt={data.alt} width={1440} height={600} className={`object-cover transition-transform duration-[25000ms] ease-in-out ${isVisible ? 'scale-125' : 'scale-100'}`} />}</SwiperSlide>
        ))}
      </Swiper>
